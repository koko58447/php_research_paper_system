...
if($action == 'show'){  
      ...
      $search = $_POST['search'];
      if($search == ''){         
          $sql="select p.*,s.Name from tblpublication p,tblstaff s where s.AID=p.TeacherID and p.TeacherID=$teacherid order by p.AID desc limit $offset,$limit_per_page";
      }else{ 
        $sql="select p.*,s.Name from tblpublication p,tblstaff s where s.AID=p.TeacherID and p.TeacherID=$teacherid and (s.Name like '%$search%' or p.title like '%$search%' or p.year like '%$search%') 
      ...
      if(mysqli_num_rows($result) > 0){
          ...
          $sql_total="";
          if($search == ''){
              $sql_total="select AID from tblpublication where TeacherID=$teacherid ";
          }else{
            $sql_total="select p.AID from tblpublication p,tblstaff s
            where p.TeacherID=s.AID and p.TeacherID=$teacherid and (s.Name like '%$search%' or p.Title like '%$search%' or p.year like '%$search%' ) 
            order by p.AID desc";
            }
                    ...
                    <div class='row'>                        
                        <div class='col-md-6'>
                            <div class='form-group'>
                                ...
                                <input type='text' name='teacher1' readonly class='form-control' placeholder='Enter Title' value='{$teachername}'>
                            </div>
                        </div>
                        <div class='col-md-6'>
                            <div class='form-group'>
...
if($action=='excel'){
    ...
    $search = $_POST['ser'];
      if($search == ''){         
          $sql="select p.*,s.Name from tblpublication p,tblstaff s where s.AID=p.TeacherID and p.TeacherID=$teacherid order by p.AID desc";
      }else{ 
        $sql="select p.*,s.Name from tblpublication p,tblstaff s where s.AID=p.TeacherID and p.TeacherID=$teacherid and (s.Name like '%$search%' or p.title like '%$search%' or p.year like '%$search%') 
        order by p.AID desc";
      }

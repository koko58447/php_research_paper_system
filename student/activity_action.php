...
if($action == 'show_activity'){  
    ...
    if($search == ''){         
        ...
        where a.StudentID=s.AID".$usertype." group by a.StudentID order by a.AID desc limit $offset,$limit_per_page";
    }else{ 
        $sql="select a.*,s.Name as sname from tblstudent s,tblstudent_activity a 
        where a.StudentID=s.AID".$usertype." and (s.Name like '%$search%' or a.Name like '%$search%')  
        group by a.StudentID order by a.AID desc limit $offset,$limit_per_page";  
    }
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        if($search == ''){         
            ...
            where a.StudentID=s.AID".$usertype."  group by a.StudentID order by a.AID desc";
        }else{ 
            $sql_total="select a.AID from tblstudent s,tblstudent_activity a 
            where a.StudentID=s.AID".$usertype."  group by a.StudentID and (s.Name like '%$search%' or a.Name like '%$search%')  
            order by a.AID desc";  
        }
...
if($action=='excel'){
    ...
    if($search == ''){         
        ...
        $sql="select a.*,s.Name as sname from tblstudent s,tblstudent_activity a 
        where a.StudentID=s.AID".$usertype." and (s.Name like '%$search%' or a.Name like '%$search%')  
        group by a.StudentID order by a.AID desc";  
    }
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "AllStudentActivityReport".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '
        ...
        <table >  
            <tr>
                    <td colspan="4" align="center"><h3>All Student Activity Lists</h3></td>
            </tr>

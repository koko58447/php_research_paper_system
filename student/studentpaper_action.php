...
if($action == 'show'){  
    ...
    $sid=$_SESSION['sid'];
      ...
      if($search == ''){         
          ...
          where r.StudentID=s.AID and r.PaperID=p.AID and r.StudentID=$sid 
        ...
        ,r.StudentID,r.PaperID,s.Name as sname,p.Name as pname from 
        tblresult r,tblstudent s,tblpaper p 
        where r.StudentID=s.AID and r.PaperID=p.AID and r.StudentID=$sid  and (s.Name like '%$search%' or 
         p.Name like '%$search%') 
         group by r.StudentID,r.PaperID,s.Name,p.Name  
        order by r.AID desc limit $offset,$limit_per_page";
...
if($action=='excel'){
    ...
    $out="";
    $fileName = "StudentPaperReport".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '<head><meta charset="utf-8" /></head>
        <table >  
            <tr>
                    <td colspan="6" align="center"><h3>Student Paper Show List</h3></td>
            </tr>
